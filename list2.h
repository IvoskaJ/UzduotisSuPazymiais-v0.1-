...
void workWithGeneratedFile(int studentuSkaicius) {
    ...
    copy(studentai.begin(), studentai.end(), std::back_inserter(malacius));
        ...
        for (it = malacius.begin(); it != malacius.end(); it++) {
            if (it->galutinis(it->egzaminas, 10, &it->ND) < 5) {
                malacius.erase(it--);
            }
        }
    ...
    visuDuomenuSkaiciavimas(studentai, studentai.size(), file);
