...
void workWithGeneratedFile(int studentuSkaicius) {
	...
	copy(studentas.begin(), studentas.end(), std::back_inserter(malacius));

	int i = 0;
	while (i != studentas.size()) {
	...
	int j = 0;
	while (j != malacius.size()) {
		if (malacius.at(j).galutinis(malacius.at(j).egzaminas, 10, &malacius.at(j).ND) < 5) {
			malacius.erase(malacius.begin() + j);
		}
		else { j++; }
	}
	...
	visuDuomenuSkaiciavimas(studentas, studentas.size(), file);
